...
def get_api_doc(apidoc_file_path: str = "mock_ApiDoc.jsonld") -> HydraDoc:
    """
    ...
    API_NAME = (
        os.environ["API_NAME"]
        if "API_NAME" in dict(os.environ).keys()
        else "creditrisk_api"
    )
...
def generate_private_borrowers(apidoc: HydraDoc, API_NAME: str):
    ...
    for borrower in range(1000):
        ...
        put_request = requests.put(
            "http://localhost:8080/{api_name}/PrivateBorrower/".format(
                api_name=API_NAME
            ),
            json=dummy_object,
        )
...
def generate_residential_mortgage(apidoc: HydraDoc, API_NAME: str):
    ...
    for mortgage in range(100):
        ...
        put_request = requests.put(
            "http://localhost:8080/{api_name}/ResidentialMortgage/".format(
                api_name=API_NAME
            ),
...
def generate_propertycollateral(apidoc: HydraDoc, API_NAME: str):
    ...
    for collateral in range(100):
        ...
        put_request = requests.put(
            "http://localhost:8080/{api_name}/PropertyCollateral/".format(
                api_name=API_NAME
            ),
...
def generate_forbearance(apidoc: HydraDoc, API_NAME: str):
    ...
    for forbearance in range(100):
        ...
        put_request = requests.put(
            "http://localhost:8080/{api_name}/Forbearance/".format(api_name=API_NAME),
...
def generate_externalcollection(apidoc: HydraDoc, API_NAME: str):
    ...
    for externalcollection in range(100):
        ...
        put_request = requests.put(
            "http://localhost:8080/{api_name}/ExternalCollection/".format(
                api_name=API_NAME
            ),
...
def generate_enforcement(apidoc: HydraDoc, API_NAME: str):
    ...
    for enforcement in range(100):
        ...
        put_request = requests.put(
            "http://localhost:8080/{api_name}/Enforcement/".format(api_name=API_NAME),
